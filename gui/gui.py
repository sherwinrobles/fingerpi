...
class App:
    def __init__(self, master, img = './demo.png', *args, **kwargs):
        ...
        # self.list_ports = apply(tk.OptionMenu, (self.master, self.var_port) + tuple(self.OPTIONS_PORT))
        ...
        self.button_open = tk.Button(self.master, text = 'Open', command=self.callback_open)
        ...
        self.button_enroll = tk.Button(self.master, text = 'Enroll', command=self.callback_enroll)
        self.button_enroll.grid(column = 1, row = 4, sticky = tk.E + tk.W)

        self.button_verify = tk.Button(self.master, text = 'Verify (1:1)', command=self.callback_verify)
        self.button_verify.grid(column = 1, row = 5, sticky = tk.E + tk.W)

        self.button_identify = tk.Button(self.master, text = 'Identify (1:N)', command=self.callback_identify)
        self.button_identify.grid(column = 1, row = 6, sticky = tk.E + tk.W)

        # self.button_verify_template = tk.Button(self.master, text = 'Verify Template', command=self.callback_verify_template)
        # self.button_verify_template.grid(column = 1, row = 7, sticky = tk.E + tk.W)

        # self.button_identify_template = tk.Button(self.master, text = 'Identify Template', command=self.callback_identity_template)
        # self.button_identify_template.grid(column = 1, row = 8, sticky = tk.E + tk.W)

        self.button_ispressfinger = tk.Button(self.master, text = 'Is Press Finger', command=self.callback_ispressfinger)
        self.button_ispressfinger.grid(column = 1, row = 9, sticky = tk.E + tk.W)

        # self.button_get_image = tk.Button(self.master, text = 'Get Image', command=self.callback_get_image)
        # self.button_get_image.grid(column = 1, row = 10, sticky = tk.E + tk.W)

        # self.button_get_raw_image = tk.Button(self.master, text = 'Get Raw Image', command=self.callback_get_raw_image)
        # self.button_get_raw_image.grid(column = 1, row = 11, sticky = tk.E + tk.W)

        # self.button_cancel = tk.Button(self.master, text = 'Cancel', command=self.callback_cancel)
        # self.button_cancel.grid(column = 1, row = 12, sticky = tk.E + tk.W)

        # self.button_get_live_image = tk.Button(self.master, text = 'Get Live Image', command=self.callback_get_live_image)
        ...
        self.button_close = tk.Button(self.master, text = 'Close', command=self.callback_close)
    ...
    def callback_ispressfinger(self):
        response = self.fp.IsPressFinger()
